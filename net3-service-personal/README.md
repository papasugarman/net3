# Net3 Personal Service

A secure WebSocket-based personal page service using the Net3 communications protocol with post-quantum cryptography (ML-KEM and Dilithium).

## Description

This service provides personal information and portfolio content over a secure, authenticated WebSocket connection. It implements a custom secure protocol with:
...
The server will:
- Start listening on `ws://localhost:80/personal` (default)
...
WebSocket server listening on 0.0.0.0:80/personal

Server ready! Waiting for connections...
WebSocket URL: ws://localhost/personal
----------------------------------------
```
...
Clients must:
- Connect to the WebSocket endpoint (`ws://localhost/personal`)
...
- `1` - Get owner info (name, server ID, client ID, timestamp)
- `2` - Get personal page (HTML with biography and project information)
...
2. Check correct URL path: `ws://localhost/personal` (not `ws://localhost` or `ws://localhost:80`)
